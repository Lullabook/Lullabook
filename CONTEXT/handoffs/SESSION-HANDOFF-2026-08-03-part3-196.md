...
Scoped ESLint and `git diff --check` passed. Full `npm run verify` was not used as the ticket gate because the shared worktree has concurrent later-wave changes; parent/grader must re-run the broad gate on the settled tree and judge only this commit's files.
