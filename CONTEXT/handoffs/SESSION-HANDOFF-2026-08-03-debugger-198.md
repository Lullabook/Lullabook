...
Scoped ESLint had 0 errors and 3 existing unused-symbol warnings in `story-cap.ts`; `git diff --check` passed. Broad verify must be rerun after the shared later-wave work settles. The reviewer should inspect the #198 diff independently, including the boundary coverage outside the focused fake-provider tests.
