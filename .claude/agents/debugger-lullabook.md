...
- **Issue format + tracker:** this project's /planner issue template (see
...
4. **Per bug — frame, then fix:**
   - **Frame** it as a /planner-format issue: name the violated invariant and write a
...
5. **Report back to /debugger:** bugs found (by net), bugs fixed (gates green), and every
   unfixed follow-up. **Do not review your own diff** — a separate fresh-eyes checker
