...
- **Start here:** `CONTEXT/ROUTER.md` — the map. Tells you which index to read
  so you can find an answer without reading all of `CONTEXT/`. Then
  `CONTEXT/state.md` for what the last session left behind.
...
- **The only permitted root-level files** are the retrieval layer, which must sit
  above the categories to route between them: `CONTEXT.md` (glossary),
  `ROUTER.md`, `state.md`, and the generated `index*.md`. Do not add others.
- The `index*.md` files are **generated** — never hand-edit them. After adding,
  renaming, or deleting anything under `CONTEXT/`, run `npm run graph:index`.
